...
from app.utils.logging_utils_local import setup_logger, log_api_request, flush_telemetry

# Set up logger for the SaaS application
logger = setup_logger(
    name="saas",
    log_level="DEBUG",
    enable_app_insights=False,  # Disable App Insights
    component="saas"
)
...
class RequestTimingMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        # Skip user ID check since we don't have AuthenticationMiddleware
        user_id = None
...
async def saas_root(request: Request):
    """
    Serve the SaaS index HTML page.
    ...
    Returns:
        HTML response
    """
    return templates.TemplateResponse("saas/index.html", {"request": request})
...
if __name__ == "__main__":
    ...
    uvicorn.run("app.main_saas_local:app", host=host, port=port, reload=True)

...
def run_migrations(resource_group: str, web_app_name: str) -> bool:
    ...
    command = f"curl -s -w \"\\n%{{http_code}}\" -X POST -u \"{username}:{password}\" -H \"Content-Type: application/json\" https://{web_app_name}.scm.azurewebsites.net/api/command -d \"{{\\\"command\\\":\\\"cd /home/site/wwwroot && python -m scripts.migrate\\\", \\\"dir\\\":\\\"/home/site/wwwroot\\\"}}\""
    output, success = run_command(command)
    if not success:

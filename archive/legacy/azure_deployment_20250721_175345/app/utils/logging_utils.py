...
# Optional Application Insights imports
try:
...
def get_telemetry_client() -> Optional[TelemetryClient]:
    ...
    Returns:
        TelemetryClient instance or None if instrumentation key is not set or Application Insights is not available
    """
    global _telemetry_client
    
    # Return None if Application Insights is not available
    if not APPINSIGHTS_AVAILABLE or TelemetryClient is None:
        return None
...
class AppInsightsHandler(logging.Handler if not APPINSIGHTS_AVAILABLE else LoggingHandler):
    """
    Custom Application Insights logging handler that adds component information.
    Falls back to standard logging.Handler if Application Insights is not available.
    """
    ...
    def emit(self, record):
        ...
        Args:
            record: Log record
        """
        # If Application Insights is not available, do nothing
        if not APPINSIGHTS_AVAILABLE or not self.client:
            return

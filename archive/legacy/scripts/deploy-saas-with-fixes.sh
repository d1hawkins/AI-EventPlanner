#!/bin/bash
# Deploy the AI Event Planner SaaS application to Azure with fixes for database tables and real agents
# This script is based on azure-deploy-saas-complete.sh with fixes for the two issues
...
cp run_azure_migrations_fixed_updated.py $DEPLOY_DIR/run_azure_migrations_fixed.py
cp alembic.ini $DEPLOY_DIR/

# Create scripts directory and copy the direct table creation script
mkdir -p $DEPLOY_DIR/scripts
cp scripts/create_azure_tables_direct_fixed_updated.py $DEPLOY_DIR/scripts/create_azure_tables_direct_fixed.py
...
# Create an enhanced app_adapter_with_agents.py with better error handling

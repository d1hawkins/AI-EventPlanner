...
class CoordinatorSearchTool(BaseTool):
    ...
    # Declare the private field for Pydantic compatibility
    search_service_instance: Optional[SearchService] = Field(default=None, exclude=True)
    
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        # Don't set search_service_instance here - let it use the default value from Field
        # and initialize lazily in the property
    
    @property
    def search_service(self):
        """Lazy initialization of search service to ensure it's created when needed."""
        if self.search_service_instance is None:
            self.search_service_instance = SearchService()
        return self.search_service_instance

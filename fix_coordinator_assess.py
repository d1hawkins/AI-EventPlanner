#!/usr/bin/env python3
"""
Script to modify the assess_request function in coordinator_graph.py to check for proposal approval
...
def fix_coordinator_graph():
    ...
    # Find the assess_request function
    assess_request_start = -1
    for i, line in enumerate(lines):
        if "def assess_request" in line:
            assess_request_start = i
            break
    
    if assess_request_start == -1:
        print("Error: Could not find assess_request function.")
        return False
    
    # Find where to insert our code (after the function definition and docstring)
    insert_point = -1
    for i in range(assess_request_start, len(lines)):
        if "# Get the last message" in lines[i]:
            insert_point = i
            break
    
    if insert_point == -1:
        print("Error: Could not find insertion point in assess_request function.")
        return False

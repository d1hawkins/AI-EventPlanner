#!/usr/bin/env python3
"""
Standalone Azure adapter with embedded agent functionality - ASGI Compatible
...
print("🚀 Starting standalone agent adapter (ASGI)...")
...
async def app(scope, receive, send):
    """
    ASGI application with embedded agent functionality
    """
    if scope["type"] != "http":
        return
    ...
    if path_info == '/health':
        ...
        await send({
            ...
            'headers': [
                ...
                [b'content-length', str(len(response_json)).encode()],
            ],
        })
        await send({
    ...
    # Agent message endpoint
        ...
        try:
            ...
            print(f"Agent request: {agent_type} - {message_text[:50]}...")
            
            # Get agent response
            if REAL_AGENTS_AVAILABLE and agent_factory:
                # Use real agent
                result = await agent_factory.get_agent_response(agent_type, message_text, conversation_id)
                print("✅ Real agent response generated")
            else:
                # Fallback mock response
                result = {
                    "response": f"Mock response from {agent_type} agent: {message_text}",
...
<body>
    ...
    <div class="status success">
        <h3>✅ Application Status: Running (ASGI)</h3>
...
if __name__ == "__main__":
    print("🚀 Starting standalone agent server (ASGI)...")
    import uvicorn

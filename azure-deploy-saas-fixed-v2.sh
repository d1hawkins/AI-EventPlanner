...
# Create a deployment script with explicit package installation
...
# Create a startup.sh script for Azure App Service
...
az webapp deploy --resource-group $RESOURCE_GROUP --name $APP_NAME --src-path "$DEPLOY_ZIP"

# Configure the startup command to use the startup.sh script
az webapp config set \
    --name $APP_NAME \
    --resource-group $RESOURCE_GROUP \
